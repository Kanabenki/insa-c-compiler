...
init: {
        ...
        if (instr_table_init(&table, 1024) != 0) {
            printf("[SYMBOL] Error initializing symbol table\n");
            exit(1);
        }
        if (symbol_table_init(&tab_instr, 1024) != 0) {
            printf("[SYMBOL] Error initializing instruction table\n");
            exit(1);
        }
        printf("[SYMBOL] Symbol table and instruction table initialized\n");
    } start;
...
expr: exprL tEQUAL expArth tENDINST  {
    ...
    | tIF tLPAR booleens tRPAR {
        asm_jmpc(0xFF,0xF)/*à fixer*/;
     ...
     tLCURL body tRCURL 
    | tWHILE tLPAR booleens tRPAR tLCURL body tRCURL ;
exprL: type tID {
...
expArth: tLCURL expArth tRCURL
    ...
    | expArth tBOOLEQUAL expArth {type op_type = asm_load_symbols_op(); asm_equ(0, 0, 1); symbol* sym = add_temporary_symbol(table, op_type); asm_store(sym->address, 0);}
    | expArth tINEQUAL expArth {/*changer en not equal*/ type op_type = asm_load_symbols_op(); asm_equ(0, 0, 1); symbol* sym = add_temporary_symbol(table, op_type); asm_store(sym->address, 0);}
    | expArth tSUPE expArth {type op_type = asm_load_symbols_op(); asm_supe(0, 0, 1); symbol* sym = add_temporary_symbol(table, op_type); asm_store(sym->address, 0);}
    | expArth tSUP expArth {type op_type = asm_load_symbols_op(); asm_sup(0, 0, 1); symbol* sym = add_temporary_symbol(table, op_type); asm_store(sym->address, 0);}
    | expArth tINFE expArth {type op_type = asm_load_symbols_op(); asm_infe(0, 0, 1); symbol* sym = add_temporary_symbol(table, op_type); asm_store(sym->address, 0);}
    | expArth tINF expArth {type op_type = asm_load_symbols_op(); asm_inf(0, 0, 1); symbol* sym = add_temporary_symbol(table, op_type); asm_store(sym->address, 0);}
    | expArth tBOOLEQUAL expArth
    | expArth tINEQUAL expArth
    | val;
